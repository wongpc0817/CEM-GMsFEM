...
for sec_ind in range(SEC_NUM):
    for sub_sec_ind in range(SUB_SEC_NUM):
        os_ly = 2 + sub_sec_ind
        u0_ms=np.load(f'Plots/model1_sol_{sec_ind}_{os_ly}.npy')
        u0_ref=np.load(f'Plots/model1_ref_{sec_ind}_{os_ly}.npy')
        # Create data
        x = np.linspace(0, 1, FINE_GRID)
        y = np.linspace(0, 1, FINE_GRID)
        x, y = np.meshgrid(x, y)
        fig = plt.figure()
